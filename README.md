...
Codex 的会话正文在 `~/.codex/sessions`，侧边栏索引在 `~/.codex/state_*.sqlite` 等本地状态库里。每个 JSONL 会话文件开头还有一段 `session_meta`，里面也记录了 `model_provider`。

本工具不会删除会话正文，也不会改写消息内容。切换模型时会查找已有 Codex 状态库，同步 `threads` 表里的 `model_provider` / `model`，并同步每个 JSONL 第一行 `session_meta.model_provider`，让同一批会话在 GPT 和 DeepSeek 模式下都可见。

每次切换会写入诊断日志：

```text
~/.codex/codex-model-switcher/switcher.log
```
